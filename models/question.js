...
const defToRoots = async (roots, words, word, cutToOneRoot = false) => {
  ...
  const answer = _.map(word.components, c => ({
    value: c.value,
    missing: _.some(wordRoots, r => _.isEqual(r, c))
  }));
  const answerChoices = _.map(wordRoots, r => ({
    value: r.value,
    hint: r.definition
  }));

  const redHerrings = _.sample(
    _.reject(roots, r => _.contains(_.pluck(answerChoices, "value"), r.value)),
    CHOICES_COUNT - answerChoices.length
  );
  const choices = _.map(redHerrings, h => ({
    value: h.value,
    hint: _.sample(h.definitions)
  })).concat(answerChoices);
...
const defToChars = async (roots, words, word, cutToOneRoot = false) => {
  ...
  const answerValues = _.uniq(
    _.pluck(_.filter(answer, a => a.missing), "value")
  );
  const redHerringsCount = SPELL_CHOICES_COUNT - answerValues.length;
  const redHerrings = _.sample(
    _.filter(ALPHABET, char => !_.contains(answerValues, char)),
    redHerringsCount
  );
  const choices = _.map(answerValues.concat(redHerrings).sort(), c => ({
    value: c
  }));
  choices.forEach(c => (c.value = upcase(c.value)));
...
const defCompletion = (roots, words, word) => {
  const wordRoots = _.filter(roots, r =>
    word.roots.some(wr => wr.equals(r._id))
  );

  const params = word.defCompletionParams(wordRoots);
  const redHerrings = _.sample(
    _.reject(roots, r => r.value === params.answer.hint),
    5
  );
  const choices = _.map(redHerrings, c => ({
    value: c.definitions[0],
    hint: c.value
  })).concat(params.answer);
...
const defToAllRootsNoHighlight = async (...args) =>
  await defToAllRoots(...args);
...
const defToWord = (roots, words, word) => {
  ...
  const choices = _.map(
    _.sample(_.reject(words, w => w.value === answer.value, "value"), 5).concat(
      answer
    ),
    c => _.pick(c, "value")
  );
...
const rootInWordToDef = (roots, words, word) => {
  ...
  const easyPrompt = normalPrompt.concat({
    value: ` = ${word.fullDefinition()}`,
    highlight: false
  });
  const prompts = { normal: normalPrompt, easy: easyPrompt };

  const answer = { value: _root.definition, missing: true };
  const redHerrings = _.map(
    _.sample(_.reject(roots, r => r.value === _root.value), 5),
    r => ({ value: r.definitions[0], hint: r.value })
  );
  const choices = redHerrings.concat({
    value: answer.value,
    hint: _root.value
  });
  choices.forEach(c => (c.hint = upcase(c.hint)));
...
const defToCharsAllRootsNoHighlight = async (...args) =>
  await defToCharsAllRoots(...args);
...
exports.createQuestions = async (data, words, roots) => {
  const promises = _.map(data, async elem => {
    ...
    try {
      ...
      return _.extend({}, question, {
        ...
        choices: _.contains(SPELL_TYPES, level)
          ? question.choices
  ...
  let [questions, errors] = _.partition(
