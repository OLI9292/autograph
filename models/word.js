...
wordSchema.methods.prompts = function() {
  ...
  const withDefs = _.flatten(
    _.map(this.definition, d => {
      const _root =
        d.isRoot && _.find(this.components, c => c.definition === d.value);
      const rootValue = _root && _root.value.toUpperCase();
      return d.isRoot
        ? [
            { value: d.value, highlight: true },
            { value: ` (${rootValue})`, highlight: true }
          ]
...
wordSchema.methods.defCompletionParams = function(roots) {
  ...
  const hiddenRoot = _.find(
    this.components,
    c =>
      c.componentType === "root" && hiddenRootInDef.value.includes(c.definition)
  );
  const _root = _.find(roots, r => r.value === hiddenRoot.value);

  const definition = this.fullDefinition()
    .replace(
      hiddenRootInDef.value,
      Array(hiddenRootInDef.value.trim().length)
        .fill("_")
        .join("") + " "
    )
    .trim();
  ...
  const prompt = _.flatten(
    this.highlightedComponents(_root.value).concat(
      `= ${definition}`
        .split(" ")
        .map(d => ({ value: " " + d, highlight: false }))
    )
  );

  return {
    prompt: { normal: prompt },
    answer: { value: hiddenRoot.definition, hint: hiddenRoot.value }
  };
};
