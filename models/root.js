...
const create = async (obj, wordId) => {
  const newRoot = new Root({ value: obj.value, definitions: [obj.definition], words: [wordId] })
  try {
...
rootSchema.statics.createOrUpdate = async (obj, wordId) => {
  ...
  return existing ? await update(existing, obj, wordId) : await create(obj, wordId)
}
...
rootSchema.statics.createOrUpdateMultiple = async (words) => {
  for (let i = 0; i < words.length; i++) {
    ...
    const rootComponents = word.components.filter((c) => c.componentType === 'root')
