...
const koreaDemoWords = require("../lib/koreaDemoWords")
...
const addWordTo = (words, pool, daisyChain = false) => {
  const available = _.shuffle(
    _.reject(pool, word => _.pluck(words, "value").includes(word.value))
  )
...
const randomWordsAndLevels = async (user, questionsCount) => {
  const seenWords = _.pluck(
    _.sample(user.words, Math.round(questionsCount * BATTLE_SEEN_WORDS_RATIO)),
    "name"
  )
  const unseenWordsCount = questionsCount - seenWords.length
  const unseenWords =
    unseenWordsCount > 0
      ? await Word.find({ value: { $nin: seenWords } }, { value: 1 }).limit(
          unseenWordsCount
        )
      : []
...
const questions = async params => {
  ...
  if (
    !["demo", "korea-demo", "train", "speed", "multiplayer", "battle"].includes(
      type
    )
  ) {
  ...
  if (type === "demo") {
    data = wordsAndLevels(demoWords, null, 1)
  }
  if (type === "korea-demo") {
    data = wordsAndLevels(koreaDemoWords, null, 1).map(({ word, level }) => ({
      word,
      level: `${level}-korea`
    }))
  }
  if (type === "train") {
    data = await trainData(level, stage, user)
  }
  if (type === "speed") {
    data = speedData(level, user)
  }
  if (type === "multiplayer") {
    data = wordsAndLevels(_.shuffle(seed.split(",")), user)
  }
  if (type === "battle") {
    data = await randomWordsAndLevels(user, questionsCount)
  }
...
exports.read = async (req, res, next) => {
  ...
  if (params.error) {
    return res.status(422).send(params)
  }
