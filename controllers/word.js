  ...
  if (data) {
    ...
    Word.find({ value: { $in: values }}, (error, words) => {
      ...
      Word.find({ roots: { $in: roots }}, (error, words) => {
        ...
        const [searched, related] = _.partition(words, (w) => _.contains(values, w.value))

        const result = {}

        searched.forEach((s) => {
          result[s.value] = related
            .filter((r) => _.intersection(r.roots.map((x) => x.toString()), s.roots.map((x) => x.toString())).length > 0)
            .map((r) => r.value)
        })
...
exports.delete = (req, res, next) => {
  Word.findOneAndRemove({ _id: req.params.id }, async (error, word) => {
    ...
    if (word) {
      Root.update({ words: word._id }, { $pull: { words: word._id } }, { multi: true })
